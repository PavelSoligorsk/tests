...
async def generate_ai_test(
    ...
    Принимает список групп и/или отдельных учеников, промпт (описание 
    нужной темы), количество заданий, сложность и период исключения 
    недавно пройденных задач. AI классифицирует темы, подбирает задания 
    и создаёт готовый тест.
    
    Параметры запроса (JSON):
    - prompt (str, обязательно): описание темы/запроса, например 
      "квадратные уравнения, 10 класс"
    - task_count (int, 1-50, по умолчанию 10): количество заданий в тесте
    - difficulty (str, опционально): "easy" / "medium" / "hard", 
      если не указано — все уровни
    - group_ids (list[int], опционально): ID групп учеников, 
      можно указать несколько; ученики дедуплицируются
    - student_ids (list[int], опционально): ID отдельных учеников, 
      можно комбинировать с group_ids
    - recent_weeks (float, по умолчанию 1.5): период в неделях, 
      за который исключать уже решённые задания; 0 = не исключать
    
    Бизнес-логика:
    1. AI-классификатор определяет темы и разделы из промпта
    2. Фильтрация по сложности
    3. Исключение заданий, решённых учениками за последние recent_weeks недель
    4. Подбор заданий по темам (с fallback по ключевым словам и случайным)
    5. AI выбирает лучшие задания из отфильтрованных
    6. Создаётся тест с флагом is_ai_generated=True
    
    Возвращает: объект теста (TestResponse) с tasks, creator_id и метаданными.
    
    Ошибки:
    - 400: не указаны ученики, группа не найдена, нет заданий вне периода и т.д.
    - 403: доступ запрещён (не учитель и не админ)
    """
    try:
        result = await service.generate_ai_test(
            ...
            recent_weeks=payload.recent_weeks,
        )
