...
from models import Task, Test, TestTaskAssociation, User, UserAnswer, TestAssignment, TestResult
import requests
...
async def send_task_to_tg(
    ...
    diff = int(task.difficulty) if task.difficulty is not None else 1
...
def assign_student_to_teacher(
    ...
    db.query(models.TeacherStudent).filter(
        models.TeacherStudent.student_id == data.student_id
    ).delete()
