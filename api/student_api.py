...
def get_ai_solution(
    ...
    solve_prompt = """
Ты — AI-репетитор по математике. Реши задачу и дай полное, подробное решение.
...
=== ТРЕБОВАНИЯ ===
1. Реши задачу пошагово с подробными вычислениями
2. В конце напиши: "=== ОТВЕТ === ..."
   - Если выбор варианта — напиши номер / номера варианта (например: "=== ОТВЕТ === 1" или "=== ОТВЕТ === 2,3")
   - Если открытый ответ — напиши число или выражение (например: "=== ОТВЕТ === 42" или "=== ОТВЕТ === -46")
3. Используй $...$ для выражений в тексте и $$...$$ для формул и вычислений
4. Не используй списки, маркеры, звёздочки
5. Пиши связным текстом, но с выделением шагов решения
6. В формулах ВСЕГДА используй двойной обратный слеш: \\cdot, \\frac, \\sqrt и т.д.

=== ПРИМЕР ОФОРМЛЕНИЯ ===

Рассмотрим задачу нахождения суммы наибольшего и наименьшего значений функции $f(x) = \\frac{{3}}{{5}}x^5 - x^3 + \\frac{{4}}{{5}}$ на отрезке $[-2; 0,5]$. В конце результат нужно умножить на 5.

Шаг 1: Найдем производную функции $f(x)$.

Производная функции поможет определить критические точки, в которых могут достигаться экстремумы.

$$
f(x) = \\frac{{3}}{{5}}x^5 - x^3 + \\frac{{4}}{{5}}
$$

Вычислим производную $f'(x)$:

$$
f'(x) = \\frac{{d}}{{dx}}\\left(\\frac{{3}}{{5}}x^5 - x^3 + \\frac{{4}}{{5}}\\right) = \\frac{{3}}{{5}} \\cdot 5x^4 - 3x^2 = 3x^4 - 3x^2
$$

Шаг 2: Найдем критические точки.

Критические точки — это значения $x$, при которых производная равна нулю или не существует. Приравняем её к нулю:
...
Шаг 3: Вычислим значения функции в критических точках и на концах отрезка $[-2; 0,5]$.

Вычисляем $f(-2)$, $f(-1)$, $f(0)$, $f(0.5)$ и находим наибольшее и наименьшее значения.

$$
f(-2) = \\frac{3}{5}(-2)^5 - (-2)^3 + \\frac{4}{5} = -\\frac{96}{5} + 8 + \\frac{4}{5} = -\\frac{52}{5} = -10,4
$$

$$
f(-1) = \\frac{3}{5}(-1)^5 - (-1)^3 + \\frac{4}{5} = -\\frac{3}{5} + 1 + \\frac{4}{5} = \\frac{6}{5} = 1,2
$$
...
$$
f(0.5) = \\frac{3}{5}(0.5)^5 - (0.5)^3 + \\frac{4}{5} = \\frac{3}{160} - \\frac{1}{8} + \\frac{4}{5} = \\frac{111}{160} \\approx 0,69375
$$

Шаг 4: Найдем сумму наибольшего и наименьшего значений и умножим на 5.

Наибольшее значение: $f(-1) = \\frac{6}{5}$, наименьшее: $f(-2) = -\\frac{52}{5}$.

Сумма:
$$
\\frac{6}{5} + \\left(-\\frac{52}{5}\\right) = -\\frac{46}{5}
$$
...
"""
    ...
    def normalize_answer(text):
        if not text:
            return ""
        # Убираем пробелы, скобки, точки, приводим к нижнему регистру
