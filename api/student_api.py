...
→ Нужны ВСЕ разделы с уравнениями
...
→ Квадратные уравнения — конкретный раздел, Проценты — конкретный раздел
Ответ: {{"topics": [
...
Пример 3:
Запрос: "подготовка по геометрии"
→ НУЖНЫ ВСЕ разделы геометрии
Ответ: {{"topics": [{{"name": "Геометрия", "sections": []}}]}}

Пример 4:
Запрос: "дроби"
→ В теме "Числа и вычисления" есть несколько разделов про дроби, нужны ТОЛЬКО они
Ответ: {{"topics": [{{"name": "Числа и вычисления", "sections": ["Обыкновенные дроби", "Десятичные дроби"]}}]}}

Пример 5:
Запрос: "вся математика"
→ Нужны ВСЕ основные темы, без указания разделов
Ответ: {{"topics": [
  {{"name": "Числа и вычисления", "sections": []}},
  {{"name": "Уравнения и неравенства", "sections": []}},
  {{"name": "Геометрия", "sections": []}}
]}}

=== ИНСТРУКЦИЯ ===
1. Внимательно прочитай запрос студента
2. Выбери наиболее подходящие темы из структуры БД
3. **ПРАВИЛО ДЛЯ sections:**
   - `sections: []` → берём ВСЕ разделы этой темы
   - `sections: ["Раздел 1", "Раздел 2"]` → берём ТОЛЬКО указанные разделы
4. Если запрос очень общий ("геометрия", "математика", "вся программа") → оставь `sections: []`
5. Если запрос про "уравнения" → возьми ВСЕ разделы со словом "уравнение" (указывай их явно)
6. НЕ ПРИДУМЫВАЙ темы, которых нет в структуре БД

Верни ТОЛЬКО JSON строго в формате:
{{"topics": [{{"name": "Название темы", "sections": []}}]}}
"""
