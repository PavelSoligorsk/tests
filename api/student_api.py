...
Ответ: {{"topics": [{{"name": "Уравнения и неравенства", "sections": ["Квадратные уравнения", "Логарифмические уравнения", "Показательные уравнения"]}}]}}

Пример 2:
Запрос: "хочу квадратные уравнения и проценты"
Ответ: {{"topics": [{{"name": "Уравнения и неравенства", "sections": ["Квадратные уравнения"]}}, {{"name": "Числа и вычисления", "sections": ["Проценты"]}}]}}

Пример 3:
Запрос: "подготовка к экзамену по геометрии"
Ответ: {{"topics": [{{"name": "Геометрия", "sections": []}}]}}
(пустые sections = взять ВСЕ разделы этой темы)

Пример 4:
Запрос: "дроби и проценты"
Ответ: {{"topics": [{{"name": "Числа и вычисления", "sections": ["Обыкновенные дроби", "Десятичные дроби", "Проценты"]}}]}}

Пример 5:
Запрос: "стереометрия, пирамиды и цилиндры"
Ответ: {{"topics": [{{"name": "Геометрия", "sections": ["Пирамида", "Цилиндр", "Конус"]}}]}}

Пример 6:
Запрос: "логарифмы"
Ответ: {{"topics": [{{"name": "Уравнения и неравенства", "sections": ["Логарифмические уравнения"]}}, {{"name": "Выражения и их преобразования", "sections": ["Логарифмические выражения"]}}]}}

=== ИНСТРУКЦИЯ ===
1. Внимательно прочитай запрос студента
2. Выбери наиболее подходящие темы и разделы из структуры БД
3. Если запрос общий ("геометрия", "уравнения") — оставь sections пустым []
4. Если запрос точный — укажи конкретные разделы
5. Если запрос про "отработку уравнений" — возьми ВСЕ разделы со словом "уравнение"
6. НЕ ПРИДУМЫВАЙ темы, которых нет в структуре БД

Верни ТОЛЬКО JSON строго в формате:
{{"topics": [{{"name": "Название темы", "sections": ["Раздел 1", "Раздел 2"]}}]}}
"""
